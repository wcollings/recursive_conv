...
proc SARA:Init {instance k0 k0j p0 p0j p1 p1j p2 p2j p3 p3j q0 q0j q1 q1j q2 q2j q3 q3j} {
	...
	if [qcomplex::real{p0} == 0 && qcomplex::imag{p0} == 0] {
	...
	set order 3
	while {i < 4} {
		if [qcomplex::real{p$i} == 0 && qcomplex::imag{p$1} == 0] {
			set order [$i - 1]
		}
	}
...
proc SARA:Step {instance time input}   {   
	global SARA
	 ...
	 set delta_n [expr {$time - [lindex [dict get $solver tt] 0]}]
	 set xx_Curr [expr {[lindex [dict get $solver xx] 0]+[$delta_n]*[$input - [dict get $solver xx_Prev]]}]
	 set tempList [list 0 0 0]
	 set order [dict get $SARA $instance order]
	 set final [expr {[dict get $SARA k0]*$xx_Curr}]
	 ...
	 for {set i 0} {$i < $order} {incr i} {
		 ...
		 set q [[q[$order]] $sigma $delta_n 0]
		 incr temp [expr{$a*$q*$xx_Curr}] 
		 for {set j 0} {$j < [$order - 1]} {incr j} {
			 set q [[q[$order]] $sigma $delta_n [expr{$j+1}]]
