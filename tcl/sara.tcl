...
proc SARA:Init {instance k0 k0j p0 p0j p1 p1j p2 p2j p3 p3j q0 q0j q1 q1j q2 q2j q3 q3j args} {
	global SARA
	global solver
	set i 1
	if {$p0 == 0 && $p0j == 0} {
	...
	if {$args != ""} {
		puts "Found voltage I.C.: $args"
		set temp [complex $args 0]
		set solver($instance,"xx_0") $temp
		foreach xValue $solver($instance,"xx") {
			set solver($instance,"xx") $temp
		}
		set temp [complex 1 0]
		foreach tValue $solver($instance,"tt") {
			set solver($instance,"xx") $temp
		}
		
		SARA:Accept $instance -2 $args 
		SARA:Accept $instance -1 $args 
		SARA:Accept $instance 0 $args 
	}
}
